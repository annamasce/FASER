...
int main(int argc, char** argv) {
	...
	std::cout << "Usage: " << argv[0] << " <run> [maxevent] [mask]" << std::endl;
        ...
        std::cout << "   input_path                Path of input directory with reco file (default=input)" << std::endl;
        std::cout << "   output_path               Path of output directory (default=.)" << std::endl;
        std::cout << "   output_tag                Tag to be added to output path (default=None)" << std::endl;
		return 1;
	}
   ...
   // get the run number as the first argument
    ...
    if(argc>argv_index) {
        try {
            base_path = argv[argv_index++];
        } catch (const std::invalid_argument& e) {
            std::cerr << "Invalid argument for input_path: " << e.what() << std::endl;
            exit(1);
        } catch (const std::out_of_range& e) {
            std::cerr << "Out of range for input_path: " << e.what() << std::endl;
            exit(1);
        }
    }
    ...
    if(argc>argv_index) {
        try {
            output_tag += "_";
            output_tag += argv[argv_index++];
        } catch (const std::invalid_argument& e) {
            std::cerr << "Invalid argument for output_tag: " << e.what() << std::endl;
            exit(1);
        } catch (const std::out_of_range& e) {
            std::cerr << "Out of range for output_tag: " << e.what() << std::endl;
            exit(1);
        }
    }
    ...
    filename << output_path << "/Analysis_" << run_number;
    if(event_mask>0) {
        const char *mask = TPOEvent::DecodeEventMask(event_mask);
        filename << "_" << mask;
    }
    if(argv_index > 5) filename << output_tag;
    filename << ".root";
    ...
    inputfilename << base_path << "/Batch-TPORecevent_" << run_number << "_*_*.root";
