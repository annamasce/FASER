...
int main(int argc, char** argv) {
   ...
   // get the run number as the first argument
    ...
    while ((keepRunning && error == 0) && (ievent < max_event) && (n_events < maxevent_abs)) {
        ...
        error = fTcalEvent -> Load_event(base_path, run_number, ievent++, 0, POevent);
        if(error != 0) break;
        ...
        if(event_mask > 0){
            ...
            std::string mask_string = TPOEvent::DecodeEventMask(event_mask);
            if(mask_string == "nueCC"){
                mask_vector.push_back("nueCC");
                mask_vector.push_back("antinueCC");
            }
            if(mask_string == "numuCC"){
                mask_vector.push_back("numuCC");
                mask_vector.push_back("antinumuCC");
            }
            if(mask_string == "nutauCC"){
                mask_vector.push_back("nutauCC");
                mask_vector.push_back("antinutauCC");
            }
            if(mask_string == "nuNC"){
                mask_vector.push_back("nuNC");
            }

            if (std::find(mask_vector.begin(), mask_vector.end(), std::string(POevent -> reaction_desc())) == mask_vector.end()){
                std::cout << "Skipping event of type: " << POevent -> reaction_desc() << std::endl;
                continue;
            }
